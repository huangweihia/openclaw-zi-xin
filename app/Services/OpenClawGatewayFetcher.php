...
/**
 * 阿里云百炼 AI 采集服务
 * 直接调用阿里云百炼 API（复用邮件系统的 API Key）
 */
class OpenClawGatewayFetcher
{
    ...
    {
        // 使用阿里云百炼 API（和邮件系统相同的配置）
        $this->apiKey = env('DASHSCOPE_API_KEY');
        $this->baseUrl = 'https://dashscope.aliyuncs.com/api/v1';
        
        if (!$this->apiKey) {
            throw new \Exception('DASHSCOPE_API_KEY 未配置，请检查 .env 文件');
        }
    }
...
5. 直接返回 HTML 内容，不要其他文字";
    ...
    {
        $this->log("📡 调用阿里云百炼 AI (qwen-turbo)...");
    ...
    {
        Log::info("[OpenClawGatewayFetcher] {$message}");
        echo "[{}] {$message}\n";
    }
    ...
    {
        Log::error("[OpenClawGatewayFetcher] {$message}");
        echo "[❌] {$message}\n";
    }
}
